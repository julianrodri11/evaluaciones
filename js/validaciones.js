...
//            $("#panelpregunta"+x).removeClass("panel-danger panel-info panel-warning alert-danger").addClass("panel-success alert-success");
    ...
    //funcion que sirve para agregar estudiantes a una evaluacion

    function adicionar_aevaluacion(ced,nombre)
    {
		$(function()
		{
			//alert("-----------"+parametro);
			var r = confirm("Esta seguro de adicionar "+ced+" a la evaluación ");
			if (r == true) {
			    $('#contenedor_usuarios').append('<div class=row><input type=text value='+ced+' name=txtced[]> </div></br>');
			    alert("Estudiante adicionado");
    ...
    /*funciona que sirve para enviar datos al formulario*/
    function adcionar_estevaluacion()
    {
    	$(function()
    	{
...
/*funcion que sirve para validar si todas las preguntas qde la evaluacion fueron creadas */

	function validar_preguntas_con_respuesta(cantidad_preguntas,ide_eval)
	{
		$(function()
		{
			...
			var pre_con_resp=[];
	        for(var vradio=1; vradio<=cantidad_preguntas ;vradio++)
